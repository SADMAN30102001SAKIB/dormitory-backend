...
def generate_bot_response(conversation: Conversation, user_text: str) -> str:
    ...
    last_msgs = Message.objects.filter(conversation=conversation).order_by('-timestamp')[:10]
    snippet_lines = []
    for msg in reversed(last_msgs):
        snippet_lines.append(f"{msg.sender.capitalize()}: {msg.text}")
    snippet = "\n".join(snippet_lines)
    full_for_summary = f"So Far Entire Chat Summary: {prev_summary}\nRecent messages:\n{snippet}"
    ...
    conversation.save(update_fields=['summary', 'updated_at']) # for Update -> instance.save(). for create -> instance = Model.objects.create() or Model.objects.get_or_create()
