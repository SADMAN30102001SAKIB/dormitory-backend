...
from LLMintegration.vectorstore_utils import add_document_to_vectorstore, delete_document_from_vectorstore
import logging
...
def embed_post_on_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.error(f"Error in embed_post_on_save signal for Post ID {instance.id}: {e}", exc_info=True)
...
def delete_post_from_vectorstore(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(f"Error in delete_post_from_vectorstore signal for Post ID {instance.id}: {e}", exc_info=True)
...
def embed_comment_on_save(sender, instance, created, **kwargs):
    ...
    try:
        logger.info(f"Signal received: post_save for Comment ID {instance.id} on Post ID {instance.post.id}")
        doc_id = f"comment_{instance.id}"
        content_to_embed = f"Comment on post titled '{instance.post.title}': {instance.body}"
        metadata = {
            "source_type": "comment",
            "document_id": str(instance.id), # Ensure it's a string
        ...
        logger.error(f"Error in embed_comment_on_save signal for Comment ID {instance.id}: {e}", exc_info=True)
...
def delete_comment_from_vectorstore(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(f"Error in delete_comment_from_vectorstore for Comment ID {instance.id}: {e}", exc_info=True)